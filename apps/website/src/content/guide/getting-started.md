...
2. Set the **in/out** points on the timeline — the scrubber highlights the in/out window and marks each endpoint, and **playback stays inside it** (pressing play jumps to the in-point and stops at the out-point). Type a timestamp to jump the playhead, drag the scrubber (playback keeps going if it was already playing), or use the −60/−5/−1 / +1/+5/+60 second jump buttons.
